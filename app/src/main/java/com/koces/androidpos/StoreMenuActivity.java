...
public class StoreMenuActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tran.replace(R.id.frag_storemenu_main, frag_store1);
        tran.commit();
...
//                BleDeviceInfo();
        ...
        /** 하단 버튼 마우스 이벤트 처리
         * 211222 kim.jy */
        mBtnHome = (Button)findViewById(R.id.storemenu_btn_gotomain);
        mBtnStoreInfo = (Button)findViewById(R.id.storemenu_btn_storeInfo);
        mBtnEnv = (Button)findViewById(R.id.storemenu_btn_env);

        mBtnHome.setOnClickListener(v->{
            GotoMain();     //메인화면으로 이동 시키기
        });
        mBtnStoreInfo.setOnClickListener(v->{
            //현재 가맹점설정이라서 반응 하지 않는다. 그러나 특정 조건이 주어지면 여기에 코드를 추가 한다.
...
//            intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    public void setFrag(int n){    //프래그먼트를 교체하는 작업을 하는 메소드를 만들었습니다
        ...
        switch (n){
            case 0:
                tran.replace(R.id.frag_storemenu_main, frag_store1);  //replace의 매개변수는 (프래그먼트를 담을 영역 id, 프래그먼트 객체) 입니다.
                tran.commit();
                break;
            case 1:
                tran.replace(R.id.frag_storemenu_main,frag_tax1);
                tran.commit();
                break;
            case 2:
                tran.replace(R.id.frag_storemenu_main, frag_print1);  //replace의 매개변수는 (프래그먼트를 담을 영역 id, 프래그먼트 객체) 입니다.
                tran.commit();
                break;
        }
    }
